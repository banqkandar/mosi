...
<div class="container">
  <div class="row mt-3">
      <h4>Data selisih lama pengendara saat keluar</h4>
      <table class="table table-hover table-dark">
          <tr>
          <?php
              //data .. silahkan pusing sendiri wkwkw
              $i = 1;
              $array = explode("\n", file_get_contents('keluar.txt'));
      ...
      <div class="row">
          ...
          <table class="table table-hover">
            ...
            <?php for ($i=0; $i < $k2; $i++) { ?>
              <tr>
                ...
                <td><?=$xi[$i]?></td>
                <td><?=$fixi[$i]?></td>
                <td><?=number_format($xix[$i],3)?></td>
                <td><?=number_format($fiksi[$i],3)?></td>
              </tr>
            <?php }?>
          </table>
        </div>
        <div class="col-lg-12">
          ...
          <table class="table table-hover">
            ...
            <?php 
            $new_line = "\n";
            $file_pointer = './hasilSelKel.txt';
            if(!file_exists($file_pointer)) {
              foreach ($hasil as $data) {
                file_put_contents('hasilSelKel.txt', print_r($data, TRUE), FILE_APPEND); 
                file_put_contents('hasilSelKel.txt', print_r($new_line, TRUE), FILE_APPEND);
              }
            } else {
