...
<?php include "navbar.phtml";?>
  ...
  <!-- Page Content -->
    <div class="container">
    <div class="row mt-3">
        <h4>Data antar kedatangan masuk</h4>
        <table class="table table-hover table-dark">
          <tr>
          <?php
              //data .. silahkan pusing sendiri wkwkw
              $i = 1;              
              $array = explode("\n", file_get_contents('masuk.txt'));
              foreach ($array as $row){ // membuat tabel data input
                $fi++;
                if ($i % 5 != 0) { 
                 echo "<td>$row</td>"; 
                } else{
              ...
              $min = min($array); // min
              $gud = false; //
              $btBw = $min; //batas bawah
              while ($gud != true) {
                if($btBw[0] == 0) {
                  $btBw = substr($btBw, 1, $btBw.length); //batas bawah
                } else {
                  $gud = true;
                }
              }
              $max = max($array); //max
              $r = $max - $min; // max di kurangi min (jangkauan interval)         
              $k=number_format(1+3.3*log10($fi),5); //banyaknya interval
              $k2=number_format(1+3.3*log10($fi),0); //banyaknya interval tapi di bulatin ajah
              $p=number_format($r/$k,0); //panjang interval

              $fi = array();
              $btsBw = array($min); // batas bawah
              $btsAt = array($min+$p-1); //batas atas
              for ($j = 0; $j <= $k2; $j++) { // membuat interval
                $fi[] = 0;
                if ($j < $k2-1) {            
                  $btsBw[] = $btsAt[$j]+1;
                  $btsAt[] = $btsBw[$j+1]+$p-1;
              ...
              $fixi = array();
              for ($j = 0; $j <= $k2; $j++) {   
                $xi[] = ($btsBw[$j]+$btsAt[$j])/2; //mencari xi
                $fixi[] = $xi[$j]*$fi[$j]; //mencari fixi
              }
              $fixiTot = array_sum($fixi); //total fixi

              $rata2 = $fixiTot/$fiTot; //mencari rata - rata

              $xix = array();
              $fiksi = array();
              for ($j = 0; $j <= $k2; $j++) {   
                $xix[] = abs($xi[$j]-$rata2); //rumus xix
                $fiksi[] = $fi[$j]*pow($xix[$j],2); //mencari fixxi
              }
              
              $fiksiTot = array_sum($fiksi); //penjumlahan fiksi

              $deviasi = number_format(sqrt($fiksiTot/$fiTot),3); //mencari deviasi
      ...
      <div class="row">
        <div class="col-lg-4">
          ...
          <table class="table table-hover">
            <tbody>
            <tr>
                <td>Total fi(|xi-x|)^2</td><td><?=$fiksiTot;?></td>
              </tr>
              <tr>
        ...
        <div class="col-lg-12">
          ...
          <table class="table table-hover">
            ...
            <?php 
            $new_line = "\n";
            $file_pointer = './hasilSelMas.txt';
            if(!file_exists($file_pointer)) {
              foreach ($hasil as $data) {
                file_put_contents('hasilSelMas.txt', print_r($data, TRUE), FILE_APPEND); 
                file_put_contents('hasilSelMas.txt', print_r($new_line, TRUE), FILE_APPEND);
              }
            } else {
